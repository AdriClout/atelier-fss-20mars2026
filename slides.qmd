...
1.  Rappel de la semaine passée *(\~30 min)*
2.  Aller plus loin avec dplyr
3.  Visualisation : ggplot2
4.  R et l'IA : comment travailler intelligemment
5.  Analyse textuelle automatisée avec R
...
| `group_by()` + `summarise()` | Résumer par groupe | voir ci-dessous |
...
# 1. Aller plus loin avec dplyr

## Là où Étienne s'est arrêté

Étienne a couvert les **6 verbes de base**.

Aujourd'hui on ajoute des outils qui vont transformer votre façon de coder :

. . .

- `case_when()` — créer des variables catégorielles conditionnelles
- `left_join()` — fusionner deux tableaux de données
- Des pipelines plus complexes qui combinent tout ça

## mutate() avec case_when()

`case_when()` crée une variable catégorielle à partir de conditions — comme un `if/else` vectorisé.

```{r}
gapminder |>
  filter(year == 2007) |>
  mutate(
    categorie_vie = case_when(
      lifeExp >= 75 ~ "Élevée (75+)",
      lifeExp >= 60 ~ "Moyenne (60–75)",
      TRUE          ~ "Faible (< 60)"    # tous les autres cas
    )
  ) |>
  count(continent, categorie_vie)
```

. . .

**Logique :** R évalue les conditions dans l'ordre — dès qu'une est `TRUE`, il assigne la valeur.

## Résumer de façon avancée
...
gapminder |>
  ...
  summarise(
    ...
    esperance_median = median(lifeExp),
    esperance_min    = min(lifeExp),
    esperance_max    = max(lifeExp),
    nb_pays          = n()
  ) |>
  arrange(desc(esperance_moy))
```

## left_join() : fusionner deux tableaux

`left_join()` combine deux data frames sur une colonne commune — comme RECHERCHEV dans Excel, mais mieux.

```{r}
# Créer un tableau de codes de continents
codes_continent <- tibble(
  continent = c("Africa", "Americas", "Asia", "Europe", "Oceania"),
  code      = c("AF", "AM", "AS", "EU", "OC")
)

# Fusionner avec gapminder
gapminder |>
  filter(year == 2007) |>
  left_join(codes_continent, by = "continent") |>
  select(country, continent, code, lifeExp)
```

. . .

**Règle :** toutes les lignes du tableau de gauche sont conservées.
...
```
J'utilise R avec tidyverse. Mes données s'appellent `df` et
ont les colonnes : pays (character), annee (integer), valeur (numeric).
```
...
```
Filtre pour l'année 2020, calcule la moyenne de `valeur`
par pays, et crée un graphique en barres horizontal avec ggplot2.
```
...
```
J'ai cette erreur : [copiez l'erreur exacte].
Voici mon code : [copiez votre code].
```
...
Ajoute des commentaires expliquant chaque étape.
```

*[Démonstration live — validation du code généré ensemble]*

---

# 4. Analyse textuelle automatisée
...
## Pour aller plus loin en analyse textuelle

```{r}
# Fréquence TF-IDF (mots distinctifs par groupe)
mots_propres |>
  count(auteur, mot) |>
...
Ressource incontournable : **[Text Mining with R](https://www.tidytextmining.com/)** (Julia Silge & David Robinson) — gratuit en ligne!
