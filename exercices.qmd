...
::: {.callout-note}
Les exercices de base sur `filter()`, `select()`, `arrange()`, `group_by()` et `summarise()` sont disponibles sur le site d'Étienne : [etienneproulx.org/R_intro_fss/exercices.html](https://www.etienneproulx.org/R_intro_fss/exercices.html)

Les exercices ci-dessous **reprennent là où Étienne s'est arrêté**.
:::

## Section 1 : dplyr avancé

### Exercice 1.1 — mutate() avec case_when()

`case_when()` permet de créer une variable catégorielle à partir de conditions multiples, comme un `if/else` appliqué à toute une colonne.

En 2007, créez une colonne `cat_richesse` qui classe chaque pays en :

- `"Riche"` si `gdpPercap >= 20000`
- `"Intermédiaire"` si `gdpPercap >= 5000`
- `"Pauvre"` pour tous les autres

Ensuite, comptez combien de pays appartiennent à chaque catégorie **par continent**.

```{r}
gapminder |>
  filter(???) |>
  mutate(
    cat_richesse = case_when(
      gdpPercap >= ??? ~ "Riche",
      gdpPercap >= ??? ~ "Intermédiaire",
      TRUE             ~ "Pauvre"
    )
  ) |>
  count(???, ???)
...
gapminder |>
  ...
  mutate(
    cat_richesse = case_when(
      gdpPercap >= 20000 ~ "Riche",
      gdpPercap >= 5000  ~ "Intermédiaire",
      TRUE               ~ "Pauvre"
    )
  ) |>
  count(continent, cat_richesse)
```

**À observer :** Tous les pays d'Océanie et d'Europe sont "Riches" ou "Intermédiaires". L'Afrique est quasi entièrement "Pauvre" selon ce critère.
:::

---

### Exercice 1.2 — summarise() avancé

Calculez, pour chaque continent en **2007**, les statistiques suivantes sur l'espérance de vie :
- la moyenne (`mean`)
- la médiane (`median`)
- l'écart-type (`sd`)
...
Triez du continent avec la plus haute espérance de vie moyenne au plus bas.
...
### Exercice 1.3 — left_join() : fusionner deux tableaux

`left_join()` fusionne deux data frames à partir d'une colonne commune.

Créez ce tableau de régions géopolitiques :

```{r}
regions <- tibble(
  continent = c("Africa", "Americas", "Asia", "Europe", "Oceania"),
  region_onu = c("Global South", "Americas", "Asia-Pacific",
                 "Western nations", "Asia-Pacific")
)
```

Ensuite :

1. Fusionnez `regions` avec `gapminder` (année 2007 seulement)
2. Calculez l'espérance de vie **moyenne** par `region_onu`
3. Affichez le résultat trié

```{r}
gapminder |>
  filter(???) |>
  left_join(???, by = "continent") |>
  group_by(???) |>
  summarise(esperance_moy = mean(???)) |>
  arrange(???)
```
...
gapminder |>
  filter(year == 2007) |>
  left_join(regions, by = "continent") |>
  group_by(region_onu) |>
...
### Exercice 2.1 — facet_wrap() : petits multiples
